...
const CURI_EXECUTIVE_SLUG = "hotel-curi-executive";
const HBOOK_COMPANY_ID = "617028bb0228061225bd735e";
...
interface CuriExecutiveProviderOptions {
  bookingUrl?: string;
...
export function parseCuriBrazilianPrice(value: unknown): number | null {
...
export function buildCuriExecutiveSearchUrl(
...
function extractJsonArrayAssignment(
  ...
  const assignment = new RegExp(
    `\\bvar\\s+${variableName.replace(/[^a-zA-Z0-9_$]/g, "")}\\s*=\\s*`,
  ).exec(source);
...
function breakfastFromRate(rate: UnknownRecord): boolean | null {
  ...
  if (typeof rate.MealPlanView === "string") {
    labels.push(rate.MealPlanView);
  }
...
export function parseCuriExecutiveAvailability(
  params: ValidatedPricingSearch,
  searchUrl: string,
  payload: unknown,
  ...
  for (const rawRoom of response.Rooms) {
    ...
    for (const rawRate of room.Rates) {
      ...
      for (const [date, price] of dailyPrices) {
        ...
        if (!offers) continue;
        offers.push({
          id: `curi-executive-${roomId}-${rateId}-${date}-${params.adults}`,
          source: "official_site",
...
export class CuriExecutiveOfficialProvider
  implements OfficialSitePricingProvider
{
  readonly hotelSlug = CURI_EXECUTIVE_SLUG;
  private readonly options: CuriExecutiveProviderOptions;

  constructor(options: CuriExecutiveProviderOptions = {}) {
  ...
  async search(
    ...
    void hotel;
    const searchUrl = buildCuriExecutiveSearchUrl(
      params,
    ...
    developmentLog("CURI_EXECUTIVE_SEARCH_START", {
    ...
    try {
      ...
      const results = parseCuriExecutiveAvailability(
        params,
        searchUrl,
        availabilityPayload,
        rateMetadata,
      ...
      if (offerCounts.every((count) => count === 0)) {
        developmentLog("CURI_EXECUTIVE_UNAVAILABLE", {
          checkIn: params.checkIn,
          checkOut: params.checkOut,
          adults: params.adults,
        });
      } else {
        developmentLog("CURI_EXECUTIVE_SEARCH_SUCCESS", {
          days: results.length,
      ...
      if (error instanceof CaptchaRequiredError) {
        developmentLog("CURI_EXECUTIVE_ERROR", {
